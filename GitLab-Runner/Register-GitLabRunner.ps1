...
#>
    ...
    Begin {
        Write-Output $ErrorActionPreference -ForegroundColor Yellow
        $ErrorActionPreference = "Inquire"; # Make all errors terminating, i.e. script will ask to continue.
        Write-Output ""
        Write-Output "Debug Information"  -ForegroundColor Yellow
        Write-Output ""
        Write-Output ""
        ...
        Write-Output ""
        Write-Output "1. Step: Set right location for GitLab Runner"  -ForegroundColor Yellow
        Write-Output ""
        Write-Output "This PowerShell function is executed in $PSScriptRoot" -ForegroundColor DarkYellow
        ...
        Write-Output "Following directory is used for GitLab Runner: $var" -ForegroundColor DarkYellow

    }

    Process {
        Write-Output ""
        Write-Output "2. Step: Try to unregister, shutdown and uninstall existing runner and service."  -ForegroundColor Yellow
        Write-Output ""

        $config_toml = "$Base\etc\$UniqueId.toml"
        Write-Output "Configuration TOML file is: " $config_toml -ForegroundColor DarkYellow

        $builds_location = $var + "\$UniqueId\build"
        Write-Output "Location of build artefacts: " $builds_location -ForegroundColor DarkYellow

        $hostname = $env:computername
        $runner_name = $UniqueId + "@" + $hostname
        Write-Output "Name of runner to unregister: " $runner_name -ForegroundColor DarkYellow
        Write-Output ""
        Write-Output ""
        ...
        try {
          ...
          Write-Output "$runner_name cannot be stopped, unregistered and uninstalled because it likely does not exists" -ForegroundColor Red
          Write-Output $Error[0].Exception
        }
        Write-Output ""
        Write-Output "3. Step: Setup Win32 service $runner_name" -ForegroundColor Yellow
        Write-Output ""

        if ( -not (Test-Path $builds_location)) {
            New-Item -Path $builds_location -ItemType Directory -ErrorAction SilentlyContinue
            Write-Output "$builds_location has been created!" -ForegroundColor DarkYellow
        }
        ...
        try {
          ...
          Write-Output "GitLab Runner has been successfully installed!" -ForegroundColor DarkYellow
        } catch {
          Write-Output "$RunnerName cannot be installed" -ForegroundColor Red
          Write-Output $Error[0].Exception
        }

        Write-Output ""
        Write-Output "4. Step: Register runner $runner_name at $Site .." -ForegroundColor Yellow
        Write-Output ""

        if ( -not (Test-Path $builds_location)) {
            New-Item -Path $builds_location -ItemType Directory -ErrorAction SilentlyContinue
            Write-Output "$builds_location has been created!"
        }
        ...
        Write-Output "GitLab Runner has been successfully registered!" -ForegroundColor DarkYellow

        Try {
            & $Base\bin\$RunnerName start --service $UniqueId
            Write-Output "GitLab Runner has been successfully started!" -ForegroundColor DarkYellow
        } Catch {
            Write-Output "Caught the exception" -ForegroundColor Red
            Write-Output $Error[0].Exception
        }
    }

    End {
        Write-Output "Script has been finished" -ForegroundColor DarkYellow
    }
